        ...
        <div style={{ marginBottom: '48px', animation: 'fadeSlideIn 0.5s ease' }}>
          ...
          <div style={{ display: 'flex', gap: '24px', flexWrap: 'wrap' }}>
            {[
              ...
              { label: '< 50ms Response', color: '#00e5ff' },
            ].map((s) => (
        ...
        <div style={{
          ...
          <div style={{ flex: '1 1 280px', minWidth: '240px' }}>
            <div style={{
              ...
              ◈ HOW ENFORCEMENT WORKS IN PRODUCTION
            </div>
            <p style={{
              fontSize: '12px',
              color: '#6b9aaa',
              ...
              <span style={{ color: '#e8f4f8', fontWeight: 600 }}>Network-level enforcement</span>
              {' '}via firewall rules, browser extension, or CASB integration routes every AI call
              through the SecureAI proxy. Employees cannot bypass it any more than they can bypass HTTPS.
            </p>
          </div>

          {/* Right — deployment method cards */}
          <div style={{ flex: '1 1 320px', display: 'flex', flexDirection: 'column', gap: '10px' }}>
            {[
              {
                name: 'Firewall Rules',
                desc: 'Block direct calls to ai.google.com, api.openai.com at the network level',
                badge: 'Roadmap Q3',
              },
              {
                name: 'Chrome Extension',
                desc: 'Intercepts browser-based AI requests and tunnels through proxy',
                badge: 'Roadmap Q4',
              },
              {
                name: 'CASB Integration',
                desc: 'Enterprise CASB enforces policy for SaaS AI tools company-wide',
                badge: 'Enterprise',
              },
            ].map((m) => (
              <div key={m.name} style={{
                ...
                <div style={{ fontSize: '11px', color: '#6b9aaa', lineHeight: 1.5 }}>
                  {m.desc}
                </div>
              </div>
            ))}
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
              {THREAT_INTEL.map((t, i) => (
                ...
                >
                  ...
                  <p style={{ fontSize: '11px', color: '#6b9aaa', marginBottom: '8px', lineHeight: 1.6 }}>{t.desc}</p>
                  <div style={{
                    fontSize: '9px',
                    color: '#2a4a5a',
                    letterSpacing: '1.5px',
                    fontFamily: "'JetBrains Mono', monospace",
                  }}>DETECTION LAYER: <span style={{ color: '#00e5ff' }}>{t.layer}</span></div>
                </div>
              ))}
            </div>
          </div>
